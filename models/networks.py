...
def define_G(input_nc, output_nc, self_attention, ngf, netG, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, gpu_ids=[]):
    """Create a generator
    ...
    if netG == 'resnet_9blocks':
        net = ResnetGenerator(input_nc, output_nc, self_attention, ngf, norm_layer=norm_layer, use_dropout=use_dropout, n_blocks=9)
    elif netG == 'resnet_6blocks':
        net = ResnetGenerator(input_nc, output_nc,  self_attention, ngf, norm_layer=norm_layer, use_dropout=use_dropout, n_blocks=6)
    elif netG == 'unet_128':
...
class ResnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, self_attention, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect'):
        ...
        self.ref1 = nn.ReflectionPad2d(3)
        ...
        self.norm3 = norm_layer(ngf * mult * 2)

        mult = 2 ** n_downsampling
        self.resnetblock1 = ResnetBlock(ngf * mult, padding_type=padding_type, norm_layer=norm_layer,
                                   use_dropout=use_dropout, use_bias=use_bias)
        self.resnetblock2 = ResnetBlock(ngf * mult, padding_type=padding_type, norm_layer=norm_layer,
                                   use_dropout=use_dropout, use_bias=use_bias)
        self.resnetblock3 = ResnetBlock(ngf * mult, padding_type=padding_type, norm_layer=norm_layer,
                                   use_dropout=use_dropout, use_bias=use_bias)
        self.resnetblock4 = ResnetBlock(ngf * mult, padding_type=padding_type, norm_layer=norm_layer,
                                   use_dropout=use_dropout, use_bias=use_bias)
        self.resnetblock5 = ResnetBlock(ngf * mult, padding_type=padding_type, norm_layer=norm_layer,
                                   use_dropout=use_dropout, use_bias=use_bias)
        self.resnetblock6 = ResnetBlock(ngf * mult, padding_type=padding_type, norm_layer=norm_layer,
                                   use_dropout=use_dropout, use_bias=use_bias)
        self.resnetblock7 = ResnetBlock(ngf * mult, padding_type=padding_type, norm_layer=norm_layer,
                                   use_dropout=use_dropout, use_bias=use_bias)
        self.resnetblock8 = ResnetBlock(ngf * mult, padding_type=padding_type, norm_layer=norm_layer,
                                   use_dropout=use_dropout, use_bias=use_bias)
        self.resnetblock9 = ResnetBlock(ngf * mult, padding_type=padding_type, norm_layer=norm_layer,
                                   use_dropout=use_dropout, use_bias=use_bias)

        mult = 2 ** (n_downsampling - 0)
        self.convtran1 = nn.ConvTranspose2d(ngf * mult, int(ngf * mult / 2),
                           kernel_size=3, stride=2,
                           padding=1, output_padding=1,
                           bias=use_bias)
        self.norm4 = norm_layer(int(ngf * mult / 2))

        mult = 2 ** (n_downsampling - 1)
        self.convtran2 = nn.ConvTranspose2d(ngf * mult, int(ngf * mult / 2),
                                            kernel_size=3, stride=2,
                                            padding=1, output_padding=1,
                                            bias=use_bias)
        self.norm5 = norm_layer(int(ngf * mult / 2))

        self.ref2 = nn.ReflectionPad2d(3)
    ...
    def forward(self, input, map):
        ...
        if self.self_attention:
            ...
            if self.map_cat:
                ...
                x = self.relu(self.norm1(self.conv1(self.ref1(x*map))))
            map1 = self.conv1_map(map)

            # 64*256*256
            x = self.relu(self.norm2(self.conv2(x*map1)))
            # 128*128*128
            ...
            x = self.relu(self.norm1(self.conv1(self.ref1(x))))
