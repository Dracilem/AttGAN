...
class CycleGANModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        if is_train:
            ...
            parser.add_argument('--vgg', action='store_true', help='use vgg loss') ###
    ...
    def __init__(self, opt):
        ...
        if self.opt.vgg:
            self.loss_names = ['D_A', 'G_A', 'cycle_A', 'idt_A', 'vgg_A', 'D_B', 'G_B', 'cycle_B', 'idt_B', 'vgg_B']
        ...
        if self.opt.self_attention:
            opt.input_nc_A = 4
        self.netG_A = networks.define_G(opt.input_nc, opt.output_nc, opt.self_attention, opt.ngf, opt.netG, opt.norm,
                                        not opt.no_dropout, opt.init_type, opt.init_gain, self.gpu_ids)
        self.netG_B = networks.define_G(opt.output_nc, opt.input_nc, False, opt.ngf, opt.netG, opt.norm,
                                        not opt.no_dropout, opt.init_type, opt.init_gain, self.gpu_ids)
    ...
    def set_input(self, input):
        ...
        if self.opt.self_attention:
            self.map_A = input['A_map' if AtoB else 'B_map'].to(self.device)

    def forward(self):
        """Run forward pass; called by both functions <optimize_parameters> and <test>."""
        if self.opt.self_attention:
            self.fake_B = self.netG_A(self.real_A, self.map_A)  # G_A(A)
            ...
            self.rec_A = self.netG_B(self.fake_B, None)   # G_B(G_A(A))
            self.fake_A = self.netG_B(self.real_B, None)  # G_B(B)
            self.rec_B = self.netG_A(self.fake_A, None)   # G_A(G_B(B))
    ...
    def backward_G(self, epoch):
        ...
        if lambda_idt > 0:
            ...
            self.idt_A = self.netG_A(self.real_B, torch.ones([self.fake_A.shape[0],1,self.fake_A.shape[2],self.fake_A.shape[3]]))
            self.loss_idt_A = self.criterionIdt(self.idt_A, self.real_B) * lambda_B * lambda_idt
            # G_B should be identity if real_A is fed: ||G_B(A) - A||
            if self.opt.self_attention:
                self.idt_B = self.netG_B(self.real_A, None)
            else:
                self.idt_B = self.netG_B(self.real_A, None)
        ...
        # VGG loss
        self.loss_vgg_A = self.vgg_loss.compute_vgg_loss(self.vgg, self.fake_A,
